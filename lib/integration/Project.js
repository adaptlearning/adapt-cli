...
import { isNPM } from './PluginManagement/npm.js'
...
export default class Project {
  ...
  async isNPM () {
    if (this._isNPM !== undefined) return this._isNPM
    const isProjectNPM = fs.existsSync(path.join(this.cwd, 'src/package.json'))
    const isServerNPM = await isNPM()
    if (isProjectNPM && !isServerNPM) {
      throw new Error('Project is NPM, registry server is BOWER')
    }
    return (this._isNPM = isProjectNPM)
  }
