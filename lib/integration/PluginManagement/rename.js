...
import { searchInfo } from './bower.js'
...
export default async function rename ({
  ...
  try {
    const oldExists = await exists({ cwd, BOWER_REGISTRY_CONFIG, pluginName: oldName })
    if (!oldExists) throw new Error(`Plugin "${oldName}" does not exist`)
    const newExists = await exists({ cwd, BOWER_REGISTRY_CONFIG, pluginName: newName })
...
async function exists ({ cwd, BOWER_REGISTRY_CONFIG, pluginName }) {
  ...
  const matches = await searchInfo({
    cwd,
