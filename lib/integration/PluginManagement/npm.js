...
export function correctVersionNumbers (versionObjectHash) {
  versionObjectHash = { ...versionObjectHash }
  for (const version in versionObjectHash) {
    if (semver.valid(version)) continue
    delete versionObjectHash[version]
  }
  return versionObjectHash
}
...
export async function fetchAllInfo ({
  ...
  for (const url of BOWER_REGISTRY_CONFIG.search) {
    try {
      ...
      data.versions = correctVersionNumbers(data.versions)
