...
export default async function update ({
  ...
  if (updateTargetsToBeUpdated.length) {
    await forEachPromiseProgress({
      ...
      iterator: target => target.update(),
      progress: percentage => logger?.logProgress?.(`${chalk.bold.cyan('<info>')} Updating plugins ${percentage}% complete`)
    })
...
async function getUpdateTargets ({ project, plugins, isDryRun, isInteractive }) {
  ...
  if (!isAll) {
    const filtered = {}
    for (const target of targets) {
      const type = await target.getType()
      if (!type) continue
...
async function loadPluginData ({ logger, project, targets }) {
  ...
  await promiseAllProgress({
    promises: targets.map(target => target.markUpdateable()),
