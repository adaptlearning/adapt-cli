...
export default async function install ({
  ...
  if (installTargetsToBeInstalled.length) {
    await forEachPromiseProgress({
      ...
      iterator: target => target.install({ clone: dev }),
      progress: percentage => logger?.logProgress?.(`${chalk.bold.cyan('<info>')} Installing plugins ${percentage}% complete`)
    })
  ...
  await summariseInstallation({ logger, targets })
}
...
async function loadPluginData ({ logger, project, targets }) {
  ...
  await promiseAllProgress({
    promises: targets.map(target => target.fetchSourceInfo()),
  ...
  await promiseAllProgress({
    promises: targets.map(target => target.findCompatibleVersion(frameworkVersion)),
  ...
  await promiseAllProgress({
    promises: targets.map(target => target.markInstallable()),
 ...
 * @param {[Target]} options.targets
 */
function summariseInstallation ({ logger, targets }) {
