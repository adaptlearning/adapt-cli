 ...
 * @returns
 */
async function interactiveConflictResolution (plugins) {
...
function summariseInstallation ({ plugins }) {
  ...
  const successMsg = (installErrored.length === 0 && missing.length === 0)
    ? allSuccess
    : someSuccess
  if (installSucceeded.length === 0) console.log(chalk.cyanBright(noSuccess))
