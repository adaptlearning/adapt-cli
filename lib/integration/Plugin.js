...
import { TYPES, defaultType } from './PluginTypes.js'
...
process.on('beforeExit', () => Plugin.instances.forEach(plugin => plugin.clearExtractedSource()))
...
export default class Plugin {
  ...
  async findCompatibleVersion (framework) {
    ...
    const getMatchingVersion = async () => {
      ...
      if (!this.isPresent || !this.isVersioned) return null
