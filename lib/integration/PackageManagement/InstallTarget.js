...
export const BOWER_REGISTRY_URL = (function getRegistry () {
  ...
  if (fs.existsSync(path.join(process.cwd(), './.bowerrc'))) {
...
export default class InstallTarget extends Plugin {
   ...
   */
  constructor (name, requestedVersion, isCompatibleEnabled = process.argv.includes('--compatible')) {
  ...
  async getType (config = {}) {
    ...
    config.registry = config.registry ?? BOWER_REGISTRY_URL
    const keywords = await this.getKeywords(this.toString(), config)
  ...
  async getInfo (config = {}) {
    ...
    return (this._info = await new Promise((resolve, reject) => {
      bower.commands.info(this.toString(), null, config)
        .on('end', resolve)
        .on('error', reject)
    }))
  }
  ...
  async getKeywords (config = {}) {
    ...
    return (this._keywords = await new Promise((resolve, reject) => {
      bower.commands.info(this.toString(), 'keywords', config)
        .on('end', resolve)
        .on('error', reject)
    }))
  }
  ...
  async getRepositoryUrl (config = {}) {
    ...
    return (this._repositoryUrl = await new Promise((resolve, reject) => {
      bower.commands.lookup(this.toString(), config)
        .on('end', resolve)
        .on('error', reject)
    }))
  }
  ...
  async install () {
    const type = await super.getType()
    const outputPath = path.join(process.cwd(), 'src', type.belongsTo)
    try {
      await fs.rm(outputPath, { recursive: true })
    } catch (err) {
      throw new Error('There was a problem writing to the target directory')
    }
