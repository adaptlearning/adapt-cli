...
export default class Target extends Plugin {
  ...
  async install ({ clone = false } = {}) {
    ...
    // todo: clone / dev install for npm
    if (isNPM && clone) {
      const repoDetails = await this.getRepositoryUrl()
      if (!repoDetails) throw new Error('Error: Plugin repository url could not be found.')
      await fs.ensureDir(path.resolve(this.cwd, 'src/dev'))
      const pluginPath = path.resolve(this.cwd, 'src/dev', this.name)
      await fs.rm(pluginPath, { recursive: true, force: true })
      const url = repoDetails.url.replace(/^git:\/\//, 'https://')
      try {
        const exitCode = await new Promise((resolve, reject) => {
      ...
      await this.refetchProjectInfo()
      this.sourcePath = pluginPath
    }
    this.isNPMInstall = true
  }
  ...
  async postInstall () {
    ...
    const outputPath = path.join(this.cwd, 'src')
    const pluginPath = path.join(outputPath, 'node_modules', this.name)
    const bowerJSON = await fs.readJSON(path.join(pluginPath, 'package.json'))
    bowerJSON.version = bowerJSON.version ?? this.versionToApply
    await fs.writeJSON(path.join(pluginPath, '.package.json'), bowerJSON, { spaces: 2, replacer: null })
    await this.refetchProjectInfo()
  }
