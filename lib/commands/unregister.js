...
module.exports = {
    unregister: function (renderer) {
        ...
        // arg1 is either a repo in the form owner/name or the registered name of a plugin
        var arg1 = arguments.length >= 3 ? arguments[1] : null;
        // if arg1 is the registered name of a plugin then arg2 must be a GitHub endpoint
        var arg2 = arguments.length >= 4 ? arguments[2] : null;

        log(chalk.yellow('This will unregister the plugin at', bower.config.registry.publish));
        
         getProperties(arg1, arg2)
        .then(confirmTarget)
        .then(validateRepository)
        .then(authorize)
        .then(login)
        .then(confirm)
        .then(unregister)
        ...
        .catch(function (err) {
            log(chalk.red(err));
            log('The plugin was not be unregistered.');
            done(err);
        })
        .done();
    }
...
function getProperties(arg1, arg2) {
    var parsed;
    var errStr = "Please supply a target in the format <owner/name> or <plugin name> <GitHub endpoint>";

    if (arg2) {
        parsed = gh(arg2);
        return parsed.repo ? Q.resolve({name:arg1, repository:'https://github.com/'+parsed.repo}) : Q.reject(errStr);
    } else if (arg1) {
        parsed = gh(arg1);
        return parsed.repo ? Q.resolve({name:parsed.name, repository:'https://github.com/'+parsed.repo}) : Q.reject(errStr);
    }
...
function confirmTarget(properties) {
    var deferred = Q.defer();
    var schema = {
        properties: {
            name: {
                description: chalk.cyan('Confirm name of plugin'),
                type: 'string',
...
function validateRepository(properties) {
    var deferred = Q.defer();
    var errStr = "Please supply a target in the format <owner/name> or <plugin name> <GitHub endpoint>";

    request({
        url: properties.repository.replace(/^git/, 'https'),
        method:'GET',
...
function unregister(properties, renderer) {
    ...
    // user (username) with OAuth (token) wants to unregister owner/name (repo) which is the registered plugin (name)
    bower.commands.unregister(gh(properties.repository).repo+'/'+properties.username+'/'+properties.name, {token:properties.token})
    .on('end', function (result) {
        //log('end', result);
        deferred.resolve();
    })
