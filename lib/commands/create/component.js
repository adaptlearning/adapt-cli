...
module.exports = function (properties) {
    ...
    properties.localDir = 'adapt-' + properties.localDir;
...
function renameVariables(properties) {
    var renameFileContentPromises = properties.component.files.map(function (file) {
        var replacePromise = replaceTextContent(file, (/((contrib-)?componentName)/g), properties.component.name);

        if (/\\js\\.*.js/.exec(file)) {
            var componentNamePascalCase = properties.component.name.toLowerCase()
                .replace(/(\b|-)\w/g, function(match) {
                    return match.toUpperCase().replace(/-/g,'');
                });
            return replacePromise.then(function() {
                return replaceTextContent(file, (/ComponentName/g), componentNamePascalCase);
            });
        } else {
