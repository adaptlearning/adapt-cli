...
module.exports = {
    rename: function (renderer) {
        ...
        if (arguments.length >= 5) {
            ...
            log(chalk.red('You must call rename with the following arguments: <GitHub endpoint> <plugin name> <new plugin name>'));
            done();
        }
    }
};
...
function checkRepositoryExists(params) {
    var deferred = Q.defer();
    var errStr = "Please supply a valid GitHub endpoint";

    request({
        url: params.repository.replace(/^git/, 'https'),
        method:'GET',
        ...
        if (err) {
            ...
            res.statusCode==404 ? deferred.reject(errStr) : deferred.resolve(params);
        }
    });
...
function rename(params) {
    var deferred = Q.defer();
    var path = 'packages/rename/'+gh(params.repository).repo+'/'+params.username+'/'+params.oldName+'/'+params.newName;
