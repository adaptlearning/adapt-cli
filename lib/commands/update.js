...
/*
    ...
    function getInfo(plugin, deferred) {
        ...
        function reportError() {
            if (plugin._bowerCmdCount < bowerCmdMaxTry) {
                logger.log(chalk.bold.magenta('<debug>'), 'Could not get info for', plugin.packageName+'.', 'Retrying.');
            } else {
                logger.log(chalk.bold.magenta('<debug>'), 'Could not get info for', plugin.packageName+'.', 'Aborting.');
            }
        }
    ...
    function printSummary() {
        ...
        if (incompatible.length > 0) logger.log(chalk.whiteBright('The following plugins are using latest compatible version:'));
