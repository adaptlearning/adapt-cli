...
import updateBowerRepo from '../promise/update.js'
...
export default function update (renderer) {
...
function discoverPlugins () {
  ...
  function discoverNamedGroup (group) {
    const srcpath = path.join(Cwd(), 'src', group)
...
function checkProposedVersion (plugin, deferred) {
  ...
  const adaptVersion = project.getFrameworkVersion()
...
function getInfo (plugin, deferred) {
  ...
  try {
    ...
    bower.commands.info(plugin.packageName + versionString, null, { registry: BOWER_REGISTRY_URL, cwd: Cwd() }).on('end', onSuccess).on('error', onFail)
  } catch (err) {
    onFail()
  }
...
function promptToUpdateIncompatible () {
  ...
  return promiseSerialize(list, function (plugin) {
...
function performUpdates () {
  ...
  return promiseSerialize(filtered, function (plugin) {
...
function verifyChanged () {
  plugins.filter(isPresent).forEach(function (plugin) {
    ...
    const p = path.join(Cwd(), 'src', plugin._belongsTo, plugin.packageName, 'bower.json')
...
function clean () {
  if (fs.existsSync(path.join(Cwd(), 'bower.json'))) {
    fs.unlinkSync(path.join(Cwd(), 'bower.json'))
  }
  return Q.resolve()
}
...
function createUpdateTask (plugin) {
  ...
  return Q.when(null, function () {
    ...
    JsonWriter.writeJSONSync(path.join(Cwd(), 'bower.json'), manifest)
