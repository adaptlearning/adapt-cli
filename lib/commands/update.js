...
module.exports = function() {
    ...
    var logger;
    // our temporary bower.json
    var bowerJson;
    // a representation of the Adapt project
    var project;
    ...
    return {
        update: function(renderer) {
            ...
            bowerJson = bowerExists ? JsonLoader.readJSONSync('bower.json') : {"name":"temporary-manifest", "dependencies":{}};
    ...
    function performUpdates() {
        var promises = plugins.filter(isPresent).filter(isToBeUpdated).map(createUpdateTask);
        var settled = 0, total = promises.length;
    ...
    function printSummary() {
        ...
        plugins.forEach(function(plugin) {
            if (plugin._shouldBeUpdated) {
                ...
                logger.log(chalk.yellow(plugin.packageName, 'was skipped'));
            }
        });
