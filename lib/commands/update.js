...
/*
    ...
    function createPlugins() {
        ...
        Object.keys(bowerJson.dependencies).forEach(function(dep) {
            var plugin = Plugin.parse(dep+'#'+bowerJson.dependencies[dep]);
    ...
    function printDryRunSummary() {
        ...
        updateAvailable.sort(byPackageName).forEach(function(plugin) {
            logger.log(chalk.reset(highlight(plugin.packageName), 'can be updated from', chalk.yellowBright(plugin._installedVersion), 'to', chalk.greenBright(plugin._proposedVersion), '(latest version is '+chalk.magentaBright(plugin._latestVersion)+')'));
        });

        if (updateAvailable.length > 0) logger.log('\n');

        updateNotAvailable.sort(byPackageName).forEach(function(plugin) {
            logger.log(chalk.reset(highlight(plugin.packageName+' @'+plugin._installedVersion)+' has no compatible update (latest version is '+chalk.magentaBright(plugin._latestVersion)+')'));
        });
    ...
    function printSummary() {
        ...
        latest.sort(byPackageName).forEach(function(plugin) {
            logger.log(chalk.reset(highlight(plugin.packageName)), 'is using the latest version ('+plugin._installedVersion+')');
        });

        if (latest.length > 0) logger.log('\n');

        incompatibleConstrained.sort(byPackageName).forEach(function(plugin) {
            logger.log(chalk.reset(highlight(plugin.packageName)), 'is using the requested version ('+chalk.yellowBright(plugin._installedVersion)+'). Latest version is', chalk.magentaBright(plugin._latestVersion));
        });

        if (incompatibleConstrained.length > 0) logger.log('\n');

        incompatible.sort(byPackageName).forEach(function(plugin) {
            logger.log(chalk.reset(highlight(plugin.packageName)), 'is using the latest compatible version ('+chalk.yellowBright(plugin._installedVersion)+'). Latest version is', chalk.magentaBright(plugin._latestVersion));
        });

        if (incompatible.length > 0) logger.log('\n');

        updated.sort(byPackageName).forEach(function(plugin) {
            logger.log(chalk.reset(highlight(plugin.packageName)), 'has been updated from', chalk.yellowBright(plugin._installedVersion), 'to', chalk.greenBright(plugin._updatedVersion)+'.', 'Latest version is', chalk.magentaBright(plugin._latestVersion));
        });
