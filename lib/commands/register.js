...
export default function register (renderer) {
  ...
  // renderer.log(chalk.yellow('This will publish this plugin to', BOWER_REGISTRY_URL));
  ...
  loadPluginProperties('./bower.json', {
    ...
    // becase we don't want to allow adapt-myPackageName if adapt-contrib-myPackageName exists
      const plugin = new Plugin(properties.name)
      const contribPlugin = new Plugin(properties.name, true)
...
function registerWithBowerRepo (plugin, repository) {
  ...
  bower.commands.register(plugin.toString(), repository.url || repository, { registry: BOWER_REGISTRY_URL })
