...
module.exports = {
    create: function (renderer, type, localDir, branch, done) {
        type = arguments.length >= 3 ? arguments[1] : Constants.DefaultCreateType;
...
function installNodeDependencies (properties) {            
    var deferred = Q.defer(),
        cwd = process.cwd();
...
function installAdaptDependencies (properties) {            
    var deferred = Q.defer(),
        cwd = process.cwd();
