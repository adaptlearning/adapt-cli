...
async function confirmOptions ({ logger, type, name, branch }) {
  ...
  if (!name) {
    propertySchema.push({
      ...
      message: 'name',
      type: 'input',
      default: DEFAULT_TYPE_NAME[type]
    })
  }
  ...
  if (!branch) {
    ...
    if (type === 'course') {
      defaultBranch = await getAdaptLatestVersion({ repository: ADAPT_FRAMEWORK })
    }
  ...
  // We aren't using completely passed variables, so add confirm to prompts
  if (propertySchema.length > 0) {
    ...
    name = name || propertySchemaResults.name
    branch = branch || propertySchemaResults.branch
  }
